...
def get_predictions(model, iterator, device):
    ...
    print()
    print(classification_report(labels.numpy(), probs.argmax(1, keepdim=True).squeeze().numpy()))

    return labels, probs


def train_loop(model, train_loader, optimizer, criterion, device, l1_norm):
    train_losses = []
    train_accs = []
...
def test_loop(model, test_iterator, criterion, device):
...
def train_ffnn(data):
    ...
    train_loop(model, train_loader, optimizer, criterion, device, False)
...
def run_ffnn_experiments(data_path):
    ...
    for vec_setup in vec_setups:
        ...
        for vec in vecs:
            print(vec)
            train_ffnn(data_path + vec_setup + "/" + vec)
            print("-----------------")
